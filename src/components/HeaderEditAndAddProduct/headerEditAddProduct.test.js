...
describe("Test boton eliminar producto del componente Header", () => {
  beforeEach(() => {
    component = render(
      <MemoryRouter>
        <HeaderEditAddProduct editProduct={true}></HeaderEditAddProduct>
      </MemoryRouter>
    );
  });

  test("El botón Eliminar debe estar en el Header cuando se esta editando un producto", () => {
    expect(screen.getByText(/eliminar/i)).toBeInTheDocument;
  });

  test("El botón Eliminar debe ejecutar la funcion al hacerse click", () => {
    let btnEliminar = screen.getByRole("button");
    userEvent.click(btnEliminar);
    expect(deleteProductFunction).toHaveBeenCalled();
  });

  test("El botón Eliminar no debe estar en el Header cuando no se esta editando un producto", () => {
    cleanup();

    render(
      <MemoryRouter>
        <HeaderEditAddProduct editProduct={false}></HeaderEditAddProduct>
      </MemoryRouter>
    );
